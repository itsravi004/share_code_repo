...
class TestSupport(unittest.TestCase):
    ...
    def test_args_from_interpreter_flags(self):
        ...
        self.check_options(
            ["-I", "-E", "-s", "-P"], "args_from_interpreter_flags", ["-I"]
        )
    ...
    def test_copy_python_src_ignore(self):
        ...
        if not os.path.exists(src_dir):
            self.skipTest(
                f"cannot access Python source code directory:" f" {src_dir!r}"
            )
        ...
        if not os.path.exists(landmark):
            self.skipTest(
                f"cannot access Python source code directory:"
        ...
        self.assertEqual(
            support.copy_python_src_ignore(src_dir, names), ignored | {"build"}
        )

        # Doc/ directory
        path = os.path.join(src_dir, "Doc")
        self.assertEqual(
            support.copy_python_src_ignore(path, os.listdir(path)),
            ignored | {"build", "venv"},
        )
